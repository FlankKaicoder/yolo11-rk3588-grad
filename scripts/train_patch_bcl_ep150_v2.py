...
EXP_NAME = "patch_bcl_resnet18_fix_nosampler_ce_ep150_v2"
...
def save_config():
    config = {
        ...
        "lambda_ce": LAMBDA_CE,
        "lambda_bcl": LAMBDA_BCL,
        "device": str(DEVICE),
        "exp_name": EXP_NAME,
        "train_dir": str(TRAIN_DIR),
        "val_dir": str(VAL_DIR),
        "note": "Single-stage repaired BCL, 150 epochs, no sampler, no class-balanced CE.",
    }
...
class BalancedContrastiveLoss(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        contrast_feature = torch.cat(torch.unbind(features, dim=1), dim=0)  # [view1全体, view2全体]
...
def train_one_epoch(model, loader, ce_criterion, bcl_criterion, optimizer):
    ...
    for x1, x2, labels, _paths in loader:
        ...
        _, proj1, logits1 = model(x1)
        _, proj2, logits2 = model(x2)
